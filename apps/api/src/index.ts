...
function mapTgItem(item: {
  ...
  return {
    ...
    status: item.status.toLowerCase() as 'available' | 'reserved' | 'purchased',
  };
}
...
tgRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const items = await prisma.item.findMany({
      where: { wishlistId: id },
      orderBy: { createdAt: 'desc' },
      select: { id: true, title: true, url: true, priceText: true, priority: true, status: true },
    });
...
tgRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const itemCount = await prisma.item.count({ where: { wishlistId } });
...
// DELETE /tg/items/:id — delete item
tgRouter.delete(
  '/items/:id',
  asyncHandler(async (req, res) => {
    const id = req.params.id ?? '';
