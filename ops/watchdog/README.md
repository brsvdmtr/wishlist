...
If `flock(1)` isn't on PATH:
  - default: the wrapper logs a warning and runs unguarded (dev / CI).
  - production: set `WATCHDOG_REQUIRE_FLOCK=true` — the wrapper exits 2
    instead, refusing to start without the safety net. Recommended for
    every host where the cron schedule can produce overlapping ticks.
...
26 tests cover state transitions, persistence (round-trip + corruption +
parent-dir creation + no temp-file leaks), and zero-exposure dedup. No
...
| `WATCHDOG_REQUIRE_FLOCK` | `false` | set `true` on prod to fail-closed when `flock(1)` is unavailable |
