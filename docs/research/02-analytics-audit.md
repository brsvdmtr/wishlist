...
| 8 | **`guest.converted_to_user`** | ✅ **Wired 2026-05-27.** Emit в `apps/api/src/routes/wishlists.routes.ts:855` при создании первого regular wishlist, если `referredByUserId` или `firstAcquisitionSource ∈ SHARED_CONTENT_ACQUISITION_SOURCES`. Atribution beacon в Mini App ([`lib/attribution.ts`](../../apps/web/app/miniapp/lib/attribution.ts)) для 4 share-link путей; bot-side referral attribution в [`bot/src/referral-attribution.ts`](../../apps/bot/src/referral-attribution.ts). См. [`guest-conversion-spec.md`](./guest-conversion-spec.md). | done |
