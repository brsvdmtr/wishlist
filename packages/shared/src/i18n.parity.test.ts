...
describe('i18n content audit — no untranslated English in non-EN locales', () => {
  ...
  const INTENTIONAL_EN_KEYS = new Set<string>([
    ...
    // Showcase pure-placeholder / numeric values (wave 3)
    ...
    'paywall_hero_title',                       // "WishBoard PRO" — brand
    'paywall_save_badge',                       // "−33%" — numeric badge
  ]);
  ...
  const ES_COGNATE_KEYS = new Set<string>([
    ...
    // Referral cognates (referral_* — wave 5)
