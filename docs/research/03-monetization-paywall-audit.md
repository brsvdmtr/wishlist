...
**Resolved 2026-05-27.** Аудит был основан на устаревшем снимке кода. Sheet существует в `MiniApp.tsx:21311-21432` (BottomSheet `showCancelSub`), рендерит 9 фич (wishlists, items, participants, comments, url, hints, subs, privacy, calendar — `cancel_feat_calendar` добавили позже, отсюда 9 а не 8), оба CTA (Keep / Cancel), support-блок и lifetime-guard (frontend `!isLifetime` + backend 409). Гэп был только в аналитике: client-side funnel (`pro_cancel.sheet_viewed/keep_clicked/confirmed`) добавлен в `PRODUCT_EVENTS` с allowlist-тестом — иначе `/tg/telemetry` silently дропал бы их (prefix `pro_cancel.` не в `ANALYTICS_EVENT_PREFIXES`). Активные доки (USER_FLOWS.md, MONETIZATION.md, research/01) выровнены на "9 features". Server-authoritative `subscription.cancelled` (billing.routes.ts:365) остаётся источником правды для revenue-дашбордов.
...
- [x] Cancel-flow bottom sheet — выяснить почему не рендерится (git log по `cancel_feat_`); восстановить или формально удалить из доки. **Fixed 2026-05-27** — аудит-снимок устарел: sheet жив в `MiniApp.tsx:21289-21404` с 9 фичами + lifetime guard. Добавлен client-funnel `pro_cancel.sheet_viewed/keep_clicked/confirmed` в `PRODUCT_EVENTS` (telemetry allowlist) с pin-тестом. Активные доки выровнены на 9. Подробнее § 8.6.
