...
type Screen = 'loading' | 'error' | 'my-wishlists' | 'wishlist-detail' | 'share' | 'guest-view';
...
function WishCardOwner({ item, onEdit, onDelete }: {
  item: Item;
...
export default function MiniApp({ apiBase, botUsername }: { apiBase: string; botUsername: string }) {
  ...
  const resetItemForm = () => {
    setItemTitle(''); setItemUrl(''); setItemPrice(''); setItemPriority(2);
    setEditingItem(null);
  };
        ...
        <div style={{ padding: '16px 20px 120px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
            ...
            <button
              ...
              style={{ ...btnPrimary, width: 'auto', padding: '8px 16px', fontSize: 13, flexShrink: 0 }}
            >
              Поделиться
            </button>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {items.map((item, i) => (
              <div key={item.id} style={{ animation: `fadeIn 0.3s ease ${i * 0.06}s both` }}>
                <WishCardOwner item={item} onEdit={openEditItem} onDelete={(w) => void handleDeleteItem(w)} />
              </div>
            ))}
