...
deep-link-вход, который не подгрузил собственные данные). Все три залочены
регресс-тестами в этом же PR.

### Ошибка

1. **Удаление без подтверждения.** В меню группы тап по «Удалить группу»
   (владелец) или «Выйти из группы» (участник) срабатывал сразу, на первый
   тап — без диалога подтверждения. Один промах = группа снесена у всех.
2. **Пустая «Главная» после битого инвайта.** Открыв инвайт-ссылку на
   удалённую/истёкшую группу, пользователь видел экран «Ссылка недействительна»,
   жал «На главную» — и попадал на домашний экран, где показано «Пока пусто»
   и 0 вишлистов, хотя у него есть свои списки.
3. **Одинаковые иконки.** В чате уведомление «👥 X вступил в твою группу» и
   инлайн-кнопка «👥 Открыть группу» вели одной и той же 👥 — выглядело как
   задвоенная иконка.

### Корень

1. Кнопки «Удалить»/«Выйти» в меню звали `remove()` / `leave()` напрямую из
   `onClick`. Деструктивное действие было первичным действием тапа.
2. **Ветка `circ_` в бутстрапе MiniApp НЕ звала `loadWishlists()`** — в отличие
   от ВСЕХ остальных deep-link-веток (`guest`, `srvy_`, `br_`, `item_`, …).
   `onExit` из «Близких» делает `setScreen('my-wishlists')`, но домашний экран
   рендерит пустое состояние по `wishlists.length === 0` — а «никогда не
   загружено» неотличимо от «реально пусто». Прецедент уже был решён для
   guest-ссылки (там явно грузят свои вишлисты даже в `.catch`, с комментарием
   «чтобы back показал свои данные, а не пустую „Пока пусто“») — но для
   circle-инвайта это пропустили.
3. Обе строки (`circle_join_notif`, `notif_open_circle_btn`) во всех 6 локалях
   начинались с одной 👥.

### Урок

- **Любой deep-link-вход, у которого есть выход «на главную», обязан заранее
  подгрузить СОБСТВЕННЫЕ данные пользователя.** Экран входа по ссылке — гость в
  своём же приложении: ссылка ведёт на чужой/служебный контекст, но кнопка
  «назад/на главную» возвращает в личный, который ещё не загружен. Пустое
  состояние, отрисованное по `length === 0`, превращает «не загрузили» в
  «у тебя ничего нет» — самый тревожный возможный экран.
- **Деструктивное действие никогда не первично на тапе.** Удаление/выход/снос
  идут через явный шаг подтверждения, а не из `onClick` пункта меню.
- **Уведомление и его кнопка-действие должны иметь разные ведущие иконки** —
  одинаковый эмодзи в сообщении и в кнопке читается как визуальный баг.

### Правило

- Когда добавляешь новую deep-link-ветку в бутстрап (`startParam.startsWith`),
  сверься с соседними ветками: если у экрана есть `onExit → my-wishlists`,
  ветка ОБЯЗАНА вызвать `loadWishlists().catch(() => {})` (фоново). Это инвариант
  всех веток, а не частность. Guard: `monolith-guards.test.ts` грепает тело
  `circ_`-ветки на наличие вызова `loadWishlists(`.
- Деструктивные действия в `services`/UI идут через confirm-стейт
  (`setConfirmDestructive(true)` → отдельный `Sheet`), не из `onClick` пункта
  меню напрямую. Копия паттерна — уже существовавший remove-member confirm.
- i18n-инвариант: ведущая иконка `*_notif`-сообщения ≠ ведущая иконка его
  inline-кнопки. Guard: `packages/shared/src/i18n.circles.test.ts` (per-locale).

### Лучший код

- Bug 2 (1 строка, owner-layer): `loadWishlists().catch(() => {})` в начале
  `circ_`-ветки `apps/web/app/miniapp/MiniApp.tsx`, с комментарием про
  «На главную»-путь. Без компенсаций в дочернем слое.
- Bug 1: `DetailView` получил `confirmDestructive`-стейт + confirm-`Sheet`
  (ветвится по `isOwner`: удалить/выйти), новые ключи `circle_delete_q` /
  `circle_leave_q` × 6 локалей. `DetailView` экспортирован для тестов.
- Bug 3: `circle_join_notif` 👥 → 🎉 × 6 локалей; кнопка остаётся 👥.
- Тесты (каждый проверен на «краснеет при откате фикса»): 4 RTL на
  confirm-флоу (owner delete/cancel/confirm + member leave) в
  `CirclesRoot.test.tsx`; монолит-guard на `loadWishlists(` в `circ_`-ветке
  (усилен с `toContain('loadWishlists')` до `/loadWishlists\s*\(/`, иначе
  упоминание в комментарии удовлетворяло проверку); 6 per-locale i18n-guard.

---

## 2026-06-01 — Circles «Близкие»: свой инвайт-линк показывал онбординг-превью + тап по карточке улетал в магазин (две перекоррекции, теперь под фронт-тестами)

Фиксы уехали в прод в PR #38 (`367d6dc`, live-test round 2) БЕЗ тестов
(фронтовый RTL-долг был помечен в `project_circles_p0_1`). Эта запись + новый
файл `CirclesRoot.test.tsx` закрывают долг постфактум: оба симптома теперь
залочены регресс-тестами.

### Ошибка

Два разных симптома из живого теста «Близких»:

1. **Свой линк → онбординг.** Участник (или владелец) группы, открыв СВОЮ же
   ссылку-приглашение, попадал на экран-превью для новичков («Тебя пригласили…
   Вступить»). Человек уже в группе — а его заставляли жать «Вступить», чтобы
   «вступить» туда, где он и так состоит.
2. **Тап → магазин.** Тап по любой карточке желания в списке участника сразу
   выкидывал во внешний магазин (`Telegram.WebApp.openLink`) — без карточки-
   деталей, без возможности глянуть описание/цену/забронировать, не покидая
   Mini App. Перекоррекция прошлого раунда, где карточки вообще не тапались.

### Корень

1. Эффект превью в `JoinView` звал `setPreview(json.preview)` безусловно — не
   ветвился по `preview.alreadyMember` ПЕРЕД тем, как показать онбординг.
   Экран-вход по deep-link обращался со всеми как с потенциальным новичком.
2. `onClick` строки желания был привязан напрямую к `openMarket(it.url)` —
   строка путала «хочу рассмотреть это желание» с «отнеси меня в магазин».
   Внешняя навигация была ПЕРВИЧНЫМ (и единственным) действием тапа.

### Урок

- **Экраны-входы по deep-link / инвайту обязаны ветвиться по членству ДО
  показа онбординга.** Инвайт-ссылка — это один URL, но две аудитории:
  будущие участники (нужен превью + интро про сюрприз) и уже-участники/владелец
  (их нужно просто впустить). Сначала ответь «кто это?», и для уже-участника
  путь по умолчанию — тихий редирект, а не повторный онбординг.
- **Строки контента внутри приложения открывают внутреннюю деталь; внешняя
  навигация — это явное вторичное действие.** Тап по строке = «покажи
  подробнее», а не «уведи из приложения». Ссылку в магазин прячь за подписанной
  кнопкой внутри детали, никогда не на первичном тапе строки. Выкидывать
  пользователя из Mini App на разведочном тапе — это утечка retention.

### Правило

- Любой экран, достижимый по deep-link (`initial={{ view: … }}`), при первом
  фетче данных обязан проверить отношение зрителя к цели (участник? владелец?
  чужой?) и выбрать ветку ДО отрисовки онбординга. Для Circles это ранний
  возврат по `alreadyMember`:
  `if (preview.alreadyMember) { onJoined(circleId); return; }` ДО `setPreview`.
- Первичный `onClick` тапабельной строки контента открывает внутреннюю
  поверхность (Sheet-деталь). `openLink` / `window.open` доступны только из
  явной кнопки (`circle_open_in_store`) внутри этой поверхности.
- Колбэк редиректа читай через ref (`useRef(onJoined)`) внутри fetch-эффекта,
  чтобы ветка членства не перезапускала фетч превью на каждый ре-рендер
  родителя (зависимости эффекта — только `[tgFetch, token]`).

### Лучший код

Новый `apps/web/app/miniapp/screens/circles/CirclesRoot.test.tsx` — 9 RTL-тестов
(vitest + jsdom), `tgFetch` замокан на JSON-формы `InvitePreview` /
`MemberWishlists`, `Telegram.WebApp.openLink` — на проверку «звался / не звался»:

- **fix #5:** уже-участник редиректится (`onJoined(circleId)`, превью-UI нет);
  новичок видит превью + CTA; битый линк → экран-ошибка, `onJoined` не зовётся.
- **fix #2:** тап по строке открывает Sheet-деталь (`openLink` НЕ зовётся);
  магазин открывается только по явной кнопке «Открыть в магазине».
- **fix #3 (смежный инвариант):** в self-view (`isSelf`) деталь-шит не рендерит
  ни контрол брони, ни чип «занято» — только фото/цена/описание/магазин/закрыть;
  со-участник видит нейтральный чип или кнопку брони, но никогда — личность
  забронировавшего.

`JoinView` и `MemberView` экспортированы из `CirclesRoot.tsx` для изолированного
монтирования (они лежат на несколько уровней навигации вглубь). Каждый тест
проверен на «краснеет при откате своего фикса» (fix #5 → 2 красных, fix #2 → 2,
fix #3 → 1), затем фиксы восстановлены.
